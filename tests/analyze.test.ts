...
test("postIssueComment dry-run targets a stable issue thread", async () => {
  ...
  assert.equal(message, "dry-run: would post trace-to-skill issue-map report to owner/repo#8");
});
...
test("demo command runs packaged scenarios without private traces", async () => {
  ...
  assert.ok(scenarios.some((scenario) => scenario.id === "subagent-orchestration"));
  ...
  assert.match(list, /subagent-orchestration/);
...
test("analyzeTargets detects Codex subagent orchestration gaps", async () => {
  const result = await analyzeTargets(["fixtures/codex-subagent-orchestration.md"]);
  const finding = result.findings.find((item) => item.kind === "codex_subagent_orchestration");
  const evidence = finding?.evidence.map((item) => item.excerpt).join("\n") ?? "";
  const report = renderCodexIssueReport(result);

  assert.ok(finding);
  assert.equal(finding.severity, "high");
  assert.match(evidence, /official subagent functionality/);
  assert.match(evidence, /agent registry/);
  assert.match(evidence, /context isolation/);
  assert.match(evidence, /reasoning_effort/);
  assert.match(evidence, /agents_config\.toml/);
  assert.match(evidence, /\.agents\/subagents\/explore_agent\.md/);
  assert.match(evidence, /MCP tool set/);
  assert.match(finding.suggestedRule, /MCP tool allowlist/);
  assert.match(finding.suggestedRule, /AGENTS\.md/);
  assert.match(finding.suggestedRule, /headless `codex exec`/);
  assert.match(report, /codex_subagent_orchestration/);
});
...
test("analyzeTargets detects Codex context visibility regressions", async () => {
  const result = await analyzeTargets(["fixtures/codex-context-visibility.md"]);
  const finding = result.findings.find((item) => item.kind === "codex_context_visibility");
  const evidence = finding?.evidence.map((item) => item.excerpt).join("\n") ?? "";
  const report = renderCodexIssueReport(result);

  assert.ok(finding);
  assert.equal(finding.severity, "high");
  assert.match(evidence, /visible context\/token usage indicator/);
  assert.match(evidence, /context-window pressure/);
  assert.match(evidence, /not a replacement for passive context awareness/);
  assert.match(evidence, /local session logs/);
  assert.match(finding.suggestedRule, /chat input area/);
  assert.match(finding.suggestedRule, /statusline/);
  assert.match(report, /codex_context_visibility/);
});

test("analyzeTargets detects Codex remote connection and SSH workspace failures", async () => {
  const result = await analyzeTargets(["fixtures/codex-remote-connection.md"]);
  const finding = result.findings.find((item) => item.kind === "codex_remote_connection");
  const evidence = finding?.evidence.map((item) => item.excerpt).join("\n") ?? "";
  const report = renderCodexIssueReport(result);

  assert.ok(finding);
  assert.equal(finding.severity, "high");
  assert.match(evidence, /Remote Development in Codex Desktop App/);
  assert.match(evidence, /remote_connections = true/);
  assert.match(evidence, /local tunnel not ready/);
  assert.match(evidence, /fs\/getMetadata/);
  assert.match(evidence, /codex-server/);
  assert.match(evidence, /ForwardAgent/);
  assert.match(finding.suggestedRule, /Settings > Connections/);
...
test("issue-heat-comment dry-run targets a stable issue thread", async () => {
  const { stdout } = await execFileAsync("node", [
    "dist/src/cli.js",
    "issue-heat-comment",
    "fixtures/github-codex-issues-export.json",
    "--window-hours",
    "8",
    "--issue-number",
    "8",
...
test("initProject scaffolds workflow without overwriting existing files", async () => {
  ...
  const first = await initProject({ cwd, comment: true, sarif: true, issueMapRepo: "openai/codex", issueMapState: "all", issueMapLimit: "75", issueMapCommentIssue: "8" });
  const second = await initProject({ cwd, comment: true, sarif: true, issueMapRepo: "openai/codex", issueMapState: "all", issueMapLimit: "75", issueMapCommentIssue: "8" });
  ...
  assert.match(issueRadarWorkflow, /id: issue-heat/);
  assert.match(issueRadarWorkflow, /mode: issue-heat/);
  assert.match(issueRadarWorkflow, /issue-heat-repo: openai\/codex/);
  assert.match(issueRadarWorkflow, /issue-heat-window-hours: "24"/);
  assert.match(issueRadarWorkflow, /issues: write/);
  assert.match(issueRadarWorkflow, /issue-map-comment: "true"/);
  assert.match(issueRadarWorkflow, /issue-map-comment-issue: "8"/);
  assert.match(issueRadarWorkflow, /issue-heat-comment: "true"/);
  assert.match(issueRadarWorkflow, /issue-heat-comment-issue: "8"/);
...
test("package metadata points npm users back to the public project", async () => {
  ...
  assert.ok(packageJson.keywords?.includes("codex-subagent-orchestration"));
  assert.ok(packageJson.keywords?.includes("codex-subagent-config"));
  assert.ok(packageJson.keywords?.includes("agents-config"));
  assert.ok(packageJson.keywords?.includes("agent-orchestration"));
...
test("initProject rejects unsafe workflow arguments", async () => {
  ...
  await assert.rejects(
    () => initProject({ issueMapCommentIssue: "0" }),
    /positive integer/
  );
});
...
test("composite action exposes Codex readiness doctor mode", async () => {
  ...
  assert.match(action, /issue-heat-issues:/);
  assert.match(action, /issue-heat-considered:/);
  assert.match(action, /issue-heat-matched:/);
  ...
  assert.match(action, /issue-map-comment:/);
  assert.match(action, /issue-map-comment-issue:/);
  assert.match(action, /issue-map-comment-repository:/);
  assert.match(action, /issue-heat-path:/);
  assert.match(action, /issue-heat-repo:/);
  assert.match(action, /issue-heat-state:/);
  assert.match(action, /issue-heat-limit:/);
  assert.match(action, /issue-heat-window-hours:/);
...
test("repository dogfoods the local Codex readiness action", async () => {
  ...
  assert.match(workflow, /id: issue-heat/);
  assert.match(workflow, /mode: issue-heat/);
...
test("published JSON schemas describe CLI result contracts", async () => {
  ...
  assert.ok((analysisSchema.$defs.findingKind as { enum: string[] }).enum.includes("codex_subagent_orchestration"));
  ...
  assert.ok((analysisSchema.$defs.findingKind as { enum: string[] }).enum.includes("codex_context_visibility"));
...
test("benchmark covers public fixture failure classes", async () => {
  ...
  assert.ok(benchmark.cases.some((item) => item.id === "codex-subagent-orchestration" && item.detectedKinds.includes("codex_subagent_orchestration")));
  ...
  assert.ok(benchmark.cases.some((item) => item.id === "codex-context-visibility" && item.detectedKinds.includes("codex_context_visibility")));
