...
Tagged Action releases build and run the CLI from `$GITHUB_ACTION_PATH`, so a workflow pinned to a release tag such as `@v0.1.61` executes that release's checked-out source instead of pulling the default branch at runtime.
