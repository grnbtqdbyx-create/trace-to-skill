...
This focused linter checks whether `AGENTS.md` exists as the canonical instruction source, whether validation commands are discoverable, whether `AGENTS.md` / `CLAUDE.md` / Cursor / Copilot guidance conflicts, whether instruction files reference missing paths or grow large enough to risk ignored guidance, and whether JSON or `.codex/config.toml` MCP configs expose risky capabilities, secrets, unresolved commands, missing `cwd` values, placeholder env vars, wrong `mcpServers` casing, or unresolved plugin placeholders.
...
MCP configs with `mcpServers`, `.mcp.json`, or project-local `.codex/config.toml` are parsed for capability hints such as filesystem, shell, browser, network, database, container, and secret-bearing environment variables. `lint-agents` also checks static startup inputs such as `command`, `cwd`, env placeholders, unresolved `$VARS`, `${CLAUDE_PLUGIN_ROOT}`-style plugin placeholders, local stdio commands without explicit `cwd`, and the common JSON `mcp_servers` / `mcpServers` casing mismatch.
...
jobs:
  codex-readiness:
    ...
    steps:
      ...
      - uses: grnbtqdbyx-create/trace-to-skill@v0.1.33
        with:
          mode: all
          doctor-threshold: "85"
          doctor-comment: "true"
...
Tagged Action releases build and run the CLI from `$GITHUB_ACTION_PATH`, so a workflow pinned to a release tag such as `@v0.1.33` executes that release's checked-out source instead of pulling the default branch at runtime.
...
- MCP config parser with explicit capability scoring and JSON/TOML static startup diagnostics
