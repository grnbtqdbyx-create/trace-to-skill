...
- Codex subagents become hard to trust because completed or closed agents remain visible, stale spawn edges stay open, child threads crowd the recent list, spawn quota is exhausted, or compaction loses prior subagent IDs; `session-audit` can summarize local subagent lifecycle signal counts without exposing transcripts.
...
- Codex resume, Desktop project history/search/sidebar rendering, archived chats, context compression, or local state migrations fail after large JSONL histories, images, tool output, stale SQLite state, short or stale `session_index.jsonl`, transcript-like sidebar titles, unindexed rollout threads, or project/thread metadata drift.
...
- `trace-to-skill sensitive-audit --format ignore --ignore-target codexignore`
