...
- Public `docs/CODEX_ISSUE_RADAR.md` demo generated from live `openai/codex` issues.
...
- Usage bucket confusion detection for Codex popovers that show 5h and weekly percentages without clear remaining/used, rolling/calendar, or account/workspace scope.
...
- `usage-doctor` / `usage-evidence` reports for scattered `/status`, reset-table, usage-limit, rapid drain experiment, token-total, prompt-cache, cached-input, and orchestration-overhead snippets.
