...
3. If the repository has agent run traces, analyze them:
...
4. If the repository lacks workflows, preview setup without writing files first:

   ```bash
   npx github:grnbtqdbyx-create/trace-to-skill init --comment --sarif --dry-run
   ```

5. Report the score, failing checks, critical findings, and the exact validation commands run.
